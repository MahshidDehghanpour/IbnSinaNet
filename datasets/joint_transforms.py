...
class JointCenterCrop(object):
    ...
    def __init__(self, size, translate:int):
    ...
    def __call__(self, imgs):
        ...
        x1 = int(round((w - tw) / 2.))
        y1 = int(round((h - th) / 2.))
        x1+=random.randint(-self.translate,self.translate)
        y1+=random.randint(-self.translate,self.translate)
...
class JointGrayscale(object):
    def __call__(self, imgs):
        return [img.convert('L') for img in imgs if img.mode == 'RGB']
