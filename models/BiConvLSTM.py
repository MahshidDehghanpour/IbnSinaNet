...
class BiConvLSTM(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, filter_size, num_layers, bidirectional=False, useCuda=False,
                 bias=False):
        super().__init__()
        self.input_size = input_size
        self.hidden_size = hidden_size
        ...
        if (bidirectional == True):
            ...
            self.num_directions = 1
        cell_list = []
        cell_list.append(BiConvLSTMCell(self.input_size, self.hidden_size, self.filter_size, bidirectional, useCuda,
                                        bias))  # the first one has a different number of input channels

        for idcell in range(1, num_layers):
            cell_list.append(BiConvLSTMCell(self.hidden_size * self.num_directions, self.hidden_size, self.filter_size,
                                            bidirectional, useCuda, bias))
...
class LstmSkip(nn.Module):
    def __init__(self, input_channels, out_channels, num_directions, lstm_kernel_size_, lstm_num_layers_, seq_size):
        super().__init__()
        self.seq_size = seq_size
        self.conv = Conv2d(input_channels, out_channels, 1)
        self.lstm = BiConvLSTM(out_channels, out_channels // num_directions, lstm_kernel_size_,
                               lstm_num_layers_, num_directions == 2, True)

    def forward(self, x):
        out = self.conv(x)
        out = forward_lstm(self.seq_size, out, self.lstm)
        return torch.cat([x, out], dim=1)


def forward_lstm(seq_size, out, lstm_module):
    out = out.view(-1, seq_size, out.size(1), out.size(2), out.size(3))
    out = out.transpose(0, 1).contiguous()
    out, state = lstm_module(out, None)
    out = out.transpose(0, 1).contiguous()
    out = out.view(-1, out.size(2), out.size(3), out.size(4))
    return out
