...
class FCDenseNet(nn.Module):
    def __init__(self, in_channels, down_layers, down_blocks, up_layers, up_blocks,
        ...
        self.first_conv = nn.Conv2d(in_channels=in_channels, out_channels=first_channels,
        ...
        for idx, block_count in enumerate(self.down_blocks):
            ...
            if not last:
                stage = FCDenseStage(down_blocks[idx], curr_ch, growth_rate, down_layers[idx], csp=True, opt=opt,
                                     decoder=False)
            else:
                stage = FCDenseStage(down_blocks[idx], curr_ch, growth_rate, down_layers[idx], csp=False, opt=opt,
                                     decoder=False)
        ...
        for idx, block_count in enumerate(self.up_blocks):
            ...
            stage = FCDenseStage(up_blocks[idx], curr_ch, growth_rate, up_layers[idx], csp=False, opt=opt, decoder=True)
            curr_ch = stage.out_channels()
            self.decoder_stages.append(stage)

        self.finalConv = nn.Conv2d(in_channels=curr_ch, out_channels=n_classes, kernel_size=1, padding=0)
    ...
    def forward(self, x):
        ...
        for idx, stage in enumerate(self.decoder_stages):
            ...
            out = self.decoder_upscales[idx](skip[:, skip_slice[0]:skip_slice[1], :, :], out)
            out = stage(out)
...
def FCDenseNet67(n_classes, grow_rate, opt):
    return FCDenseNet(
        in_channels=3, down_layers=opt.down_layers, down_blocks=opt.down_blocks, up_layers=opt.up_layers,
        up_blocks=opt.up_blocks, growth_rate=grow_rate, first_channels=48, n_classes=n_classes, opt=opt)
